...
class QLearner(object):
    ...
    def __init__(self,
        ...
        taken_q_val = tf.boolean_mask(self.q_val, mask)

        # Target Q values
        target_q_val = q_func(obs_tp1_float, self.num_actions, scope="target_q_func", reuse=False)

        if double_q:
          q_val_next = q_func(obs_tp1_float, self.num_actions, scope="q_func", reuse=True)
          argmax_q_val = tf.argmax(q_val_next, axis=1)
          mask = tf.one_hot(argmax_q_val, depth=self.num_actions, dtype=tf.bool, on_value=True, off_value=False)
          max_q_val = tf.boolean_mask(target_q_val, mask)
        ...
        self.total_error = tf.reduce_mean(huber_loss(
          taken_q_val -
          tf.stop_gradient((self.rew_t_ph + gamma * max_q_val * (1 - self.done_mask_ph)))
        ))
    ...
    def step_env(self):
        ...
        if np.random.random() < self.exploration.value(self.t) or not self.model_initialized:
          ...
          q_val = self.session.run(self.q_val,
